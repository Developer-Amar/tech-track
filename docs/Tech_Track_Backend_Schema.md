# Tech Track — Backend Schema

| | |
|---|---|
| **Companion to** | PRD, TRD, App Flow, UI/UX Brief (all v1.0) |
| **Supersedes** | The schema sketch in TRD Section 3 |
| **Version** | 1.0 |

The TRD gave a high-level shape for the data model. This document is the implementation-ready version: full DDL, security policies, the business-logic functions, and the views the app actually queries. One structural change from the TRD's sketch, explained below — everything else is the same design, just filled in.

**What changed from the TRD sketch:** `round_number` was previously scattered across `riddles`, `coding_questions`, and progress tables. It now lives in exactly one place — `checkpoints` — and everything else (`riddles`, `coding_questions`, `submissions`, `round_progress`, `proctoring_events`) references `checkpoint_id` directly. A "round" is a checkpoint; the riddle and question attached to it just come along for the ride. This also fixed a gap: there was no table linking Checkpoint Staff to the specific checkpoint(s) they're allowed to see — `checkpoint_staff_assignments` is new.

## 1. Entity Relationship Diagram

```mermaid
erDiagram
    users ||--o{ unit_members : "belongs to"
    units ||--o{ unit_members : "has"
    users ||--o{ checkpoint_staff_assignments : "assigned to"
    checkpoints ||--o{ checkpoint_staff_assignments : "staffed by"
    checkpoints ||--|| riddles : "has"
    checkpoints ||--|| coding_questions : "has"
    checkpoints ||--o{ unit_checkpoint_codes : "has"
    units ||--o{ unit_checkpoint_codes : "issued"
    coding_questions ||--o{ test_cases : "has"
    units ||--o{ submissions : "submits"
    checkpoints ||--o{ submissions : "receives"
    units ||--o{ round_progress : "tracks"
    checkpoints ||--o{ round_progress : "tracked at"
    units ||--o{ proctoring_events : "logs"
    units ||--o{ notifications : "receives"
...
**New sign-up → profile row, with the domain check as a hard backstop.** The real UX-facing domain check still happens in the app before this is ever reached; this is what stops anyone trying to skip past it directly.
...
**Once a unit is locked, its roster is frozen** — this is the DB-level guarantee behind "immutable once set":
...
**Readable code generator** — avoids `0`/`O` and `1`/`I`/`l` so codes are easy to read off a phone and type correctly under time pressure:
...
**`close_registration()`** — the single function behind the Admin panel's "close registration" button. Order matters here: unit membership changes happen *before* locking, so the lock-enforcement trigger above never fights this function.
...
**`updated_at` housekeeping**, applied to `users`, `checkpoints`, `riddles`, and `coding_questions`:
...
A helper first, so policies don't repeat the same subquery everywhere:
...
create policy "admin reads all test cases" on public.test_cases for select
  using (public.current_user_role() in ('admin','super_admin'));
-- Hidden test cases are never read through this role at all -- checking a submission
-- happens server-side using the service role key, which bypasses RLS entirely.
...
```sql
-- Ties are displayed as ties per the product decision -- last_point_at is exposed
-- for reference only and is not used as a secondary sort key here.
...
-- One row per unit with its full roster and current score -- built for the
-- Admin panel's participant management screen.
...
## 7. Seed Data Checklist (before the event, not before launch)

- One row in `event_settings` (id=1) — exists by default from the table definition.
- `checkpoints` rows for all ~10 rounds, each with its `round_number` and `location_name`.
