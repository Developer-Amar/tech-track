...
6. The question is solved in an in-browser code editor — participant's choice of C, C++, Python, or Java — with no external IDE and no requirement to leave the tab.
7. Two options:
...
- **Security:** sandboxed code execution handled entirely by Judge0's own infrastructure (no outgoing network, per-submission CPU/memory/time limits, isolated per run); OAuth-based auth means no password storage; per-team-per-checkpoint codes prevent sharing between teams.
...
- **Availability:** Supabase auto-pauses after 7 days of inactivity and needs a scheduled keep-alive ping — this must be double-checked as "awake" in the 24-48 hours before event day. The code judge (Judge0) has no such risk; it's a hosted API with no idle state on our side.
...
- **Code judge:** Judge0 CE, accessed via RapidAPI rather than self-hosted — chosen specifically because self-hosting any code-execution sandbox needs privileged container access that no card-free hosting option allows, while RapidAPI's free tier needs no card at all.
...
| 5–6 | Aug 2 – Aug 15 | Event engine — riddles, per-team codes, Judge0 integration, scoring |
| 7 | Aug 16 – Aug 22 | Admin panel — CMS, roles, leaderboard, notifications, audit log |
| 8 | Aug 23 – Aug 31 | Integration testing, Judge0 load test, full dry run, buffer |
...
| Risk | Mitigation |
|---|---|
| Supabase pauses from inactivity | Scheduled keep-alive ping; manual check 24-48 hrs before the event |
| Judge0's free-tier request quota too tight for real event-day volume | Load test with simulated submissions in Week 8; confirm current quota at RapidAPI signup rather than assuming |
